...
//show transfer order
router.get("/show_transfer_order", async (req, res) => {
    try {
        const transfer_order = await show_transfer_order();
...
var show_transfer_order = function () {
    return new Promise((resolve, reject) => {
        db.query(
            "SELECT count(*) AS qty_orders,t.transport_id,t.status, DATE_FORMAT(t.transport_date,'%d %m %Y') AS transport_date, DATE_FORMAT(t.receive_date,'%d %m %Y') AS receive_date FROM orders_transport as t, orders as o WHERE t.transport_id = o.transport_id and t.status = 'transport' or t.status='received' group by t.transport_id,t.transport_date,t.receive_date",
            (error, result) => {
...
var show_order = function (item) {
    return new Promise((resolve, reject) => {
        db.query(
            ...
            "WHERE o.transport_id ='" + item.transport_id + "'" +
            "group by o.order_id ",
