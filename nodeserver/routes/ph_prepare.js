...
var one_order = function (item) {
  return new Promise((resolve, reject) => {
    db.query(
      "SELECT o.order_id,p.name,p.surname,p.gender,p.DOB,o.status,p.email,o.patient_HN_order,m.medicine_generic,m.strenght,od.qty,m.unit " +
...
var success_prepare = function (item) {
  return new Promise((resolve, reject) => {
    ...
    db.query(
      "SELECT p.name,p.surname,p.email,m.medicine_generic,m.strenght,od.qty,m.unit,ph.pharmacy_name,DATE_FORMAT(o.due_date,'%Y %m %d') AS due_date " +
      "from orders as o " +
      "left join patients as p ON o.patient_HN_order = p.patient_HN " +
      ...
      (error, result) => {
        ...
        for (var i = 0; i < result.length; i++) {
          if(i==0){
            text_medicine = result[i].medicine_generic + " " + result[i].strenght + " " + result[i].qty + " " + result[i].unit + "\n";
          }
          else{
            text_medicine += result[i].medicine_generic + " " + result[i].strenght + " " + result[i].qty + " " + result[i].unit + "\n";
          }
        ...
        if (result[0].due_date != null) {
          ...
          var info = transporter.sendMail({
            ...
            text: "เรียนคุณ " +
              ...
              "ออเดอร์ยาของคุณได้ทำการจัดเตรียมเรียบร้อยแล้ว สามารถมารับได้ที่ร้านขายยา " + result[0].pharmacy_name + "\n" +
              "ภายในวันที่ " + due_date + "\n" +
